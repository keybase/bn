(function (){
	...
	function BigInteger (v) {
		if (typeof(v) == 'string') {
			v = bigint(v);	
		} else if (v.constructor != zed.constructor) {
	...
	BigInteger.prototype.signum = function () {
		return this.compareTo(BigInteger.ZERO);
	};
	...
	BigInteger.fromDERInteger = function (buf) {
		...
		if (buf.length == 0) { ret = BigInteger.ZERO; }
			...
			var y = BigInteger.fromBuffer(buf);
			var z = BigInteger.ONE.shiftLeft(y.bitLength())
			ret = y.subtract(z);
		}
		return ret;
	};
	...
	BigInteger.fromHex = function (x) {
		return new BigInteger(bigint(x, 16));
	};
